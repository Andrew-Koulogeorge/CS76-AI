...
class Maze:
    ...
    def has_robot(self, x, y):
        ...
        # robots are stored in an array where the x y cords alternate for a given robot
        for i in range(0, len(self.robotloc), 2):
            rx = self.robotloc[i]
            ry = self.robotloc[i + 1]
            if rx == x and ry == y:
                return True # returns true if we ever find a robot with the cords we are looking for
    ...
    def create_render_list(self):
        ...
        for index in range(0, len(self.robotloc), 2):
            ...
            renderlist[self.index(x, y)] = robotchar(robot_number) # placing a robot at this index in the maze
            robot_number += 1
...
if __name__ == "__main__":
    ...
    # print(test_maze3.is_floor(1, 0)) ### Q --> There is a robot here no? Why are we returning that there is a floor? function I guess is just checking if you dont have a wall.
